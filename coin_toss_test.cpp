...
void test_tcp_mesh_coin_toss(const unsigned int id, const unsigned int count, const char * party_file, const size_t rounds, const char * logcat, int field)
{
	if (field == 31) {
		comm_client::cc_args_t cc_args;
		cc_args.logcat = std::string(logcat) + ".ctm";
		string s = "Mersenne31";
		cc_coin_toss<ZpMersenneIntElement> cct(comm_client_factory::cc_tcp_mesh, &cc_args, s);
		...
		comm_client::cc_args_t cc_args;
		cc_args.logcat = std::string(logcat) + ".ctm";
        std::string s = "Mersenne61";
		cc_coin_toss<ZpMersenneLongElement> cct(comm_client_factory::cc_tcp_mesh, &cc_args, s);
		if(0 != cct.run(id, count, party_file, rounds, 60))
			log4cpp::Category::getInstance(logcat).error("%s: %lu round tcp mesh coin toss test failed.", __FUNCTION__, rounds);
		else
			log4cpp::Category::getInstance(logcat).notice("%s: %lu round tcp mesh coin toss test succeeded.", __FUNCTION__, rounds);
